package net.poople.extended_enchanting.block;
...
import net.poople.extended_enchanting.block.custom.ExtendedEnchantingTableBlock;

import java.util.List;

public class CustomEnchantingTableMenu extends AbstractContainerMenu {
    static final ResourceLocation EMPTY_SLOT_LAPIS_LAZULI = new ResourceLocation("item/empty_slot_lapis_lazuli");
    private final Container enchantSlots = new SimpleContainer(2) {
        @Override
        public void setChanged() {
            super.setChanged();
            CustomEnchantingTableMenu.this.slotsChanged(this);
        }
    };
    ...
    public CustomEnchantingTableMenu(int pContainerId, Inventory pPlayerInventory) {
        this(pContainerId, pPlayerInventory, ContainerLevelAccess.NULL);
    }

    public CustomEnchantingTableMenu(int pContainerId, Inventory pPlayerInventory, ContainerLevelAccess pAccess) {
        ...
        this.addSlot(new Slot(this.enchantSlots, 1, 35, 47) {
            ...
            public Pair<ResourceLocation, ResourceLocation> getNoItemIcon() {
                return Pair.of(InventoryMenu.BLOCK_ATLAS, CustomEnchantingTableMenu.EMPTY_SLOT_LAPIS_LAZULI);
            }
        });
        ...
        if (pId >= 0 && pId < this.costs.length) {
            ...
            if ((itemstack1.isEmpty() || itemstack1.getCount() < i) && !pPlayer.hasInfiniteMaterials()) {
                ...
                this.access.execute((p_327081_, p_327082_) -> {
                    ...
                    List<EnchantmentInstance> list = this.getEnchantmentList(p_327081_.enabledFeatures(), itemstack, pId, this.costs[pId]);
                    if (!list.isEmpty()) {
    ...
    private List<EnchantmentInstance> getEnchantmentList(FeatureFlagSet pEnabledFeatures, ItemStack pStack, int pEnchantSlot, int pLevel) {
        this.random.setSeed((long)(this.enchantmentSeed.get() + pEnchantSlot));
        List<EnchantmentInstance> list = EnchantmentHelper.selectEnchantment(pEnabledFeatures, this.random, pStack, pLevel, false);
    ...
    public boolean stillValid(Player pPlayer) {
        return stillValid(this.access, pPlayer, ModBlocks.EXTENDED_ENCHANTING_TABLE_BLOCK.get());
    }
